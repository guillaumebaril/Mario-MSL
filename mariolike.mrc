...
alias -l game_Launch {
  ...
  if ($engine_Init($1, %w, %h)) {
    ...
    if ($engine_JumpToMap($obj_get(game,1).starting_map)) {
      ...
      engine_SetFont $rgb(255,255,255) Arial 25
      msg_Show -1 200 4000 MARIO MARIO MARIO!!!
      engine_SetFont $rgb(255,255,255) Arial 20
      msg_Show -1 230 4000 Press G to enter map editor
  ...
  if (%m) {
    ...
    if (%imgcache) {
      ...
      if (%anim) {
        ...
        drawcopy -tn @edt $rgb(255,0,255) %imgcache @edb0 %bufx %bufy %tile.w %tile.h
      }
    }
  }
...
alias -l engine_UpdateAndRender {
  ...
  drawtext -nr @edc 16777215 arial 12 1 -2 %fps.val
}
...
alias -l engine_RenderEditor {
  ...
  drawtext -nr @edc $rgb(255,255,255) arial 10 460 22 S - Save
  drawtext -nr @edc $rgb(255,255,255) arial 10 460 34 L - Load
  drawtext -nr @edc $rgb(255,255,255) arial 10 460 46 G - Exit
...
alias -l phys_CheckCollisionTop {
  ...
  if (!%m) {
    var %dotx = $floor($calc(($1 + 7) / %tile.w))
    var %m = $hget(edig,$+(%map.id,%dotx,.,%doty,.m))
  }
...
alias -l phys_CheckCollisionBottom {
  ...
  if (!%m) {
    var %dotx = $floor($calc(($1 + 10) / %tile.w))
    var %m = $hget(edig,$+(%map.id,%dotx,.,%doty,.m))
  }
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; param: crappy expression evaluator 
; desc: 
